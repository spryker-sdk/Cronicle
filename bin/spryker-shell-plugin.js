...
stream.on('json', function (job) {
    ...
    fs.writeFileSync(script_file, bashTags + job.params.script, { mode: '775' });

    let storesSpecificData = JSON.parse(process.env.SPRYKER_STORE_SPECIFIC);

    if (typeof storesSpecificData[job.params.store] === 'undefined') {
        console.log('Job has been failed. Store specific environment settings are not found for ' + job.params.store);
        process.exit(1);
    }

    let storeSpecificData = storesSpecificData[job.params.store];

    const storeSpecificEnv = {};

    for (const key in storeSpecificData) {
        storeSpecificEnv[key] = storeSpecificData[key];
    }

    var child = cp.spawn(script_file, [], {
        cwd: process.env.SPRYKER_PROJECT_ROOT || '/data',
        stdio: ['pipe', 'pipe', 'pipe'],
        env: { ...process.env, ...storeSpecificEnv },
    });
